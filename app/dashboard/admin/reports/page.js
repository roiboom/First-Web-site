...
import { BarChart3, Users, GraduationCap, ClipboardCheck, TrendingUp, Download } from 'lucide-react';
    ...
    return (
        <div className="animate-fade-in">
            <div className="card" style={{ padding: '16px 20px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        <BarChart3 size={20} color="#3b82f6" /> {t.reports_page.title}
                    </h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{t.reports_page.subtitle}</p>
                </div>
                <button className="btn btn-outline">
                    <Download size={18} /> {t.reports_page.downloadReport}
                </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                {[
                    { label: t.admin.totalStudents, value: stats?.totalStudents, icon: Users, color: '#3b82f6' },
                    { label: t.admin.activeTeachers, value: stats?.totalTeachers, icon: GraduationCap, color: '#8b5cf6' },
                    { label: t.admin.averageAttendance, value: `${stats?.attendanceRate}%`, icon: ClipboardCheck, color: '#22c55e' },
                    { label: t.reports_page.academicPerformance, value: stats?.avgGrade, icon: TrendingUp, color: '#f59e0b' },
                ].map((c, i) => {
                    const Icon = c.icon;
                    return (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
                ...
                <div className="card" style={{ padding: '24px' }}>
                    <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '20px' }}>📊 {t.reports_page.academicPerformance}</h3>
                ...
                {/* Attendance by Class */}
                <div className="card" style={{ padding: '24px' }}>
                    <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '20px' }}>📋 {t.reports_page.attendanceTrends}</h3>
                        ...
                        return (
                            <div key={cls} style={{ marginBottom: '20px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                    ...
                                    <span style={{ fontSize: '0.85rem', color: rate > 80 ? '#22c55e' : '#f59e0b', fontWeight: 600 }}>{rate}%</span>
                                </div>
